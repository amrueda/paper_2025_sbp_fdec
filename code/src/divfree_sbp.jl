...
function SemiDiscretizationFEECSparse(N, W, D, nodes, n_elements_direction, a, b)
    ...
    return SemiDiscretizationFEECSparse(N, n_elements_direction, W_diag, D, W_2D, W_x, W_y, W_hat_2D, W_hat_2D_inv, V_x, V_y, V2_x, delta_x, delta_y, Wd_x, Wd_y, element_nodes)
end
...
function compute_rhs!(du, u, semi::SemiDiscretizationFEEC, t)
    ...
    Bz_t = semi.G1 * (semi.Winv * semi.K * Ey - Ex * transpose(semi.K) * semi.Winv) * semi.G1
    du[1] = Ex_t
    du[2] = Ey_t
    du[3] = Bz_t
    return du
            ...
            for l in 1:N
                ...
                for k in 1:N
                    if k == 1
                        ...
                        s -= Ey[offset_local + 1]
                        s -= w_N * sum(semi.D_local[N+1,r] * Ey[offset_neighbor_local + r] for r in 1:(N+1))
                        s -= w_0 * sum(semi.D_local[1,r] * Ey[offset_local + r] for r in 1:(N+1))
                        Bz_t[ind] = w_inv * s
                    else
                        Bz_t[ind] = -sum(semi.D_local[k,r] * Ey[offset_local + r] for r in 1:(N+1))
                    end 
                    if l == 1
                        s = -Ex[offset_neighbor_y + N^2 + k]
                        s += Ex[offset + k]
                        s += w_N * sum(semi.D_local[N+1,r] * Ex[offset_neighbor_y + (r-1)*N + k] for r in 1:(N+1))
                        s += w_0 * sum(semi.D_local[1,r] * Ex[offset + (r-1)*N + k] for r in 1:(N+1))
                        Bz_t[ind] += w_inv * s
                    else
                        Bz_t[ind] += sum(semi.D_local[l,r] * Ex[offset + (r-1)*N + k] for r in 1:(N+1))
                    end
                    ind += 1
                end
...
function initial_condition_nodal(semi, t)
    ...
    for j in 1:N+1
        for i in 1:N+1
            Bz[i, j] = cos(pi * x[i] + pi) * cos(pi * y[j] + pi) * cos((pi * 2 / sqrt(2)) * t)
        end
    end
    # Electric field
    ...
    for j in 1:N+1
        for i in 1:N+1
            Ex[i, j] = - 0.5 * sqrt(2) * cos(pi * x[i] + pi) * sin(pi * y[j] + pi) * sin((pi * 2 / sqrt(2)) * t)
        end
    end
    Ey = zeros(Float64, N + 1, N + 1)
    for j in 1:N+1
        for i in 1:N+1
            Ey[i, j] = 0.5 * sqrt(2) * sin(pi * x[i] + pi) * cos(pi * y[j] + pi) * sin((pi * 2 / sqrt(2)) * t)
        end
    end
        ...
        for k in 1:md
            for j in 1:N
                for i in 1:N
                    x = (k-1)*dx + nodes[i] - 1
                    y = (o-1)*dx + nodes[j] - 1
                    Bz[(o-1)*md*N^2+(k-1)*N^2+(j-1)*N+i] = cos(pi * x + pi) * cos(pi * y + pi) * cos((pi * 2 / sqrt(2)) * t)
                end
            end
        end
    end
        ...
        for k in 1:md
            for j in 1:N+1
                for i in 1:N
                    x = (k-1)*dx + nodes[i] - 1
                    y = (o-1)*dx + nodes[j] - 1
                    Ex[(o-1)*md*N*(N+1)+(k-1)*N*(N+1)+(j-1)*N+i] = - 0.5 * sqrt(2) * cos(pi * x + pi) * sin(pi * y + pi) * sin((pi * 2 / sqrt(2)) * t)
                end
            end
        end
    end
        ...
        for k in 1:md
            for j in 1:N
                for i in 1:N+1
                    x = (k-1)*dx + nodes[i] - 1
                    y = (o-1)*dx + nodes[j] - 1
                    Ey[(o-1)*md*N*(N+1)+(k-1)*N*(N+1)+(j-1)*(N+1)+i] = 0.5 * sqrt(2) * sin(pi * x + pi) * cos(pi * y + pi) * sin((pi * 2 / sqrt(2)) * t)
                end
            end
        end
    end
...
function initial_condition_projected(semi::SemiDiscretizationFEEC, t)
    ...
    for j in 1:N
        for i in 1:N+1
            Ex[i, j] = 0.5 * sqrt(2) * (cos(pi * x[i] + pi) * (1 / pi) * (cos(pi * y[j+1] + pi) -
    ...
    for j in 1:N+1
        for i in 1:N
            Ey[i, j] = 0.5 * sqrt(2) * (cos(pi * y[j] + pi) * (-1 / pi) * (cos(pi * x[i+1] + pi) -
        ...
        for k in 1:md
            for j in 1:N
                for i in 1:N
                    ...
                    Ex[(o-1)*md*N^2+(k-1)*N^2+(j-1)*N+i] = 0.5 * sqrt(2) * (cos(pi * x + pi) * (1 / pi) * (cos(pi * yp1 + pi) -
                                                                cos(pi * y + pi))) * sin((pi * 2 / sqrt(2)) * t)
                end
            end
        end
    end
        ...
        for k in 1:md
            for j in 1:N
                for i in 1:N
                    ...
                    Ey[(o-1)*md*N^2+(k-1)*N^2+(j-1)*N+i] = 0.5 * sqrt(2) * (cos(pi * y + pi) * (-1 / pi) * (cos(pi * xp1 + pi) -
                                                                cos(pi * x + pi))) * sin((pi * 2 / sqrt(2)) * t)
                end
            end
        end
    end
...
function plot_variables(semi, u; prefix="")
    ...
    p1 = heatmap(vec(semi.nodes), vec(semi.nodes), transpose(Ex_nodal), levels=20, color=:turbo)
    p2 = heatmap(vec(semi.nodes), vec(semi.nodes), transpose(Ey_nodal), levels=20, color=:turbo)
    p3 = heatmap(vec(semi.nodes), vec(semi.nodes), transpose(Bz), levels=20, color=:turbo)
    p4 = heatmap(vec(semi.nodes), vec(semi.nodes), transpose(divE), levels=20, color=:turbo)
    return plot(p1, p2, p3, p4, layout=(2, 2), title=[prefix*"Ex" prefix*"Ey" prefix*"Bz" prefix*"divE"], right_margin=10mm)
end
...
function discrete_gradient!(u, semi, dt)
    ...
    n = size(semi.W_hat)
    A = sparse([I(n[1]) spzeros(n) -0.5*dt*semi.delta_y; spzeros(n) I(n[1]) 0.5*dt*semi.delta_x; 0.5*dt*semi.Wd_x -0.5*dt*semi.Wd_y I(n[1])])
    x = reduce(vcat,u)
...
function determine_timestep(semi::SemiDiscretizationFEECSparse, cfl; implicit = false, constant = false)
    if constant
        return 1e-4
    else
...
function timedisc!(u, semi, tspan, cfl; dt_analysis = 0.1, save_visu = false, implicit = false, strong = false, constant = false)
    ...
    println(@sprintf("%5s %12s %12s %12s %12s", "iter", "time", "total_energy", "max|div(E)|", "d(Ener)/dt"))
    println(@sprintf("%5d %12.6e %12.6e %12.6e %12.6e", 0, t, energy[1], div[1], compute_denergy_dt(semi, u, du)))
    t_analysis = 0
    if save_visu
    ...
    for n in 1:100000000
        ...
        if analyze || finish
            ...
            println(@sprintf("%5d %12.6e %12.6e %12.6e %12.6e", n, t, energy[counter], div[counter], compute_denergy_dt(semi, u, du)))
            if save_visu
                plot = plot_variables(semi, u)
                savefig(plot, @sprintf("output_%.4f.png",t_analysis))
            end
...
"""
function convert_to_matrix(semi::SemiDiscretizationFEECSparse, field, cont_x::Bool, cont_y::Bool)
    md = semi.n_elements_direction
    N = semi.N
    if cont_x
        n_x = md * N + 1
    ...
    if cont_x && cont_y
        for i in 1:md
            for j in 1:md
                ...
                field_matrix[((i-1)*N + 1):(i*N),((j-1)*N + 1):(j*N)] = transpose(reshape(view(field, (offset+1):(offset+N^2)), N, N))
            end
        end
    ...
    elseif cont_x
        for i in 1:md
            for j in 1:md
                ...
                field_matrix[((i-1)*(N+1) + 1):(i*(N+1)),((j-1)*N + 1):(j*N)] = transpose(reshape(view(field, (offset+1):(offset+N*(N+1))), N, N+1))
            end
        end
        field_matrix[:, end] = view(field_matrix, :, 1)
    elseif cont_y
        for i in 1:md
            for j in 1:md
                offset = (i-1)*md*N*(N+1) + (j-1)*N*(N+1)
                field_matrix[((i-1)*N + 1):(i*N),((j-1)*(N+1) + 1):(j*(N+1))] = transpose(reshape(view(field, (offset+1):(offset+N*(N+1))), N+1, N))
            end
        end
        field_matrix[end, :] = view(field_matrix, 1, :)
    else
        for i in 1:md
            for j in 1:md
                ...
                field_matrix[((i-1)*(N+1) + 1):(i*(N+1)),((j-1)*(N+1) + 1):(j*(N+1))] = transpose(reshape(view(field, (offset+1):(offset+(N+1)^2)), N+1, N+1))
            end
        end
    end
    return field_matrix
