...
for p in degrees
    for N in (4*p):(4*p + n_number_nodes - 1)
        ...
        for i in 1:n_iterations
            ...
            timedisc!(u, semi, tspan, cfl, dt_analysis = 0.1, save_visu = false, constant = true)

            u_nodal = convert2nodal(semi, u)
            u_exact = initial_condition_nodal(semi, tspan[2])

            error_Ex_L2[i, N - 4*p + 1, p-1] = l2_norm(semi, u_exact[1] .- u_nodal[1], true, false)
            error_Ey_L2[i, N - 4*p + 1, p-1] = l2_norm(semi, u_exact[2] .- u_nodal[2], false, true)
            error_Bz_L2[i, N - 4*p + 1, p-1] = l2_norm(semi, u_exact[3] .- u_nodal[3], true, true)
        end
    end
end
